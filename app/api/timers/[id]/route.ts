...
import { timerUpdateSchema, getZodErrorMessages } from "@/lib/schemas";
...
export async function PUT(request: Request, { params }: { params: Promise<{ id: string }> }) {
  ...
  try {
     ...
     const validated = timerUpdateSchema.safeParse(body);
     ...
     return NextResponse.json({ error: "An error occurred while updating the timer. Please try again." }, { status: 500 });
  }
}
